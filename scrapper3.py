...
def display_menu():
    ...
    print(f"{Colors.YELLOW}3. {Colors.WHITE}Download images")
...
def main():
    print_logo()
    display_menu()
    choice = input(f"{Colors.GREEN}Enter your choice (1 or 3): {Colors.RESET}")
    ...
    if choice == '1':
        ...
        download_images_from_json('output_jsons', 'images');
        return
    elif choice == '0':
        exit()
    else:
...
def download_images_from_json(json_folder, output_folder):
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for i, image_url in enumerate(images):
                    try:
                        response = requests.get(image_url)
                        if response.status_code == 200:
                            image_name = f"{data['name']}_{i + 1}.jpg"
                            image_path = os.path.join(output_folder, image_name)

                            with open(image_path, 'wb') as img_file:
                                img_file.write(response.content)

                            print(f"{Colors.GREEN}Downloaded: {image_name}{Colors.RESET}")
                        else:
                            print(f"{Colors.RED}Failed to download: {image_url}{Colors.RESET}")

                    except Exception as e:
                        print(f"{Colors.RED}Error downloading {image_url}: {e}{Colors.RESET}")
