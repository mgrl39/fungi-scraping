    ...
    if isinstance(json_data, dict):
        for key, value in json_data.items():
            if isinstance(value, str):
                # Traducir el texto al catalán (idioma 'ca')
                json_data[key] = translator.translate(value, src='es', dest='ca').text
        ...
        for i in range(len(json_data)):
            if isinstance(json_data[i], str):
                json_data[i] = translator.translate(json_data[i], src='es', dest='ca').text
...
output_directory = 'output_jsons_cat'
